...
- greedy; 욕심쟁이 알고리즘. 말그대로 문제를 단순 무식하게, 탐욕적으로 푸는 알고리즘.
...
> 💡 문제풀이 핵심: 가장 큰 화폐 단위부터 거슬러 주자! 큰 단위가 항상 작은 단위의 배수 형태 -> 큰 단위부터 거슬러주어야 최적의 해를 찾을 수 있다.
...
<!-- 예를 들어 순서대로 2, 4, 5, 4, 6으로 이루어진 배열이 있을 때, M이 8이고 K가 3이라면 이 경우 특정한 인덱스의 수가 연속해서 세 번까지만 더해질 수 있으므로 큰 수의 법칙에 따른 결과는 6 + 6 + 6 + 5 + 6 + 6 + 6 + 5 = 46이 된다. 단, 서로 다른 인덱스에 해당하는 수가 같은 경우에도 서로 다른 것으로 간주한다. 배열의 크기 N, 숫자가 더해지는 횟수 M, 그리고 K가 주어질 때 동빈이의 큰 수의 법칙에 따른 결과를 출력하시오. -->
